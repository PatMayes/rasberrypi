...
Instead of creating the directories one by one, you can issue a one liner using a the option <b>mkdir -p</b>, with the <b>p</b> option being a parent or paths after the parent. So for example, you could have just created all the directories by doing the following:<br><br>
...
Note the error message under scripts test section. This section is used for unit testing which will be covered in future topics. 
